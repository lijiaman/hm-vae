...
def get_interp_target_vis_rendered_mesh_only(out_folder, rot_mat, root_trans, temporal_mask,\
     orig_width=2048, orig_height=1024, orig_img=False, wireframe=False, sideview=False, dest_name_tag=None):
    # rot_mat: T X 24 X 3 X 3
    # root_trans: T X 3
    timesteps = rot_mat.shape[0]
    renderer = Renderer(resolution=(orig_width, orig_height), orig_img=orig_img, wireframe=wireframe)

    output_img_folder = os.path.join(out_folder, "images_bg")
    ...
    # init_cam = torch.from_numpy(np.asarray([0.5, 0.5, 0, 0])).unsqueeze(0)
    init_cam = torch.from_numpy(np.asarray([0.3, 0.3, 0, 0])).unsqueeze(0)

    # Color is BGR!
    # mesh_color = np.asarray([255, 51, 153])/255.0 # Light purple
    # mesh_color = np.asarray([255, 153, 51])/255.0
    # mesh_color = np.asarray([255, 51, 51])/255.0
    # mesh_color = [0.2, 0.6, 1.0] mesh color:(1.0, 0.5, 0.6395888557866587)
    img = np.ones((orig_height, orig_width, 3))*255
    mesh_color = np.asarray([255, 51, 51])/255.0 
    add_floor = False   
    cnt = 0
    for frame_idx in range(timesteps):
        if temporal_mask[frame_idx] == 1:
            ...
            mc = mesh_color
            # print("mesh color:{0}".format(mesh_color))

            mesh_filename = None
            
            mesh_folder = os.path.join(out_folder, 'meshes')
            ...
            cnt += 1
            # print("init camera:{0}".format(init_cam.size()))
            img = renderer.render(
                img,
                pred_vertices[0].data.cpu().numpy(),
                cam=init_cam[0].data.cpu().numpy(),
                color=mc,
                angle=90,
                axis=[1,0,0],
                render_bg=True,
    ...
    cv2.imwrite(os.path.join(output_img_folder, "interp_bg.png"), img)
    # shutil.rmtree(output_img_folder)
    # os.rmdir(output_img_folder)

    return img

def vis_rendered_mesh_only(out_folder, rot_mat, root_trans, temporal_mask=None, bg_img=None, orig_width=2048, orig_height=1024, \
    orig_img=False, wireframe=False, sideview=False, dest_name_tag=None):
    # rot_mat: T X 24 X 3 X 3
    # root_trans: T X 3
    timesteps = rot_mat.shape[0]
    renderer = Renderer(resolution=(orig_width, orig_height), orig_img=orig_img, wireframe=wireframe)

    output_img_folder = os.path.join(out_folder, "images")
    os.makedirs(output_img_folder, exist_ok=True)
    ...
    # print("mesh color:{0}".format(mesh_color))
    # mesh_color = [1.0, 0.5, 0.8] # purple
    # mesh_color = np.asarray([102, 0, 102])/255.0 # Dark purple, good!
    # mesh_color = np.asarray([0, 0, 102])/255.0 # Dark orange, dark red, good?
    # mesh_color = np.asarray([204, 0, 0])/255.0 # Bright blue,ok.
    # mesh_color = np.asarray([204, 0, 102])/255.0
    # mesh_color = np.asarray([255, 51, 153])/255.0 # Light purple
    # mesh_color = np.asarray([255, 153, 51])/255.0
    # mesh_color = np.asarray([255, 51, 51])/255.0
    # mesh_color = [0.2, 0.6, 1.0] mesh color:(1.0, 0.5, 0.6395888557866587)
    for frame_idx in range(timesteps):
        if temporal_mask is None:
            mesh_color = np.asarray([255, 51, 153])/255.0
        else:
            if temporal_mask[frame_idx] == 1:
                mesh_color = np.asarray([255, 51, 51])/255.0
            else:
    ...
    # ========= Save rendered video ========= #
    if dest_name_tag is None:
        save_name = "test_vis.mp4"
    else:
        save_name = dest_name_tag + ".mp4"
...
def for_interpolation_vis_rendered_mesh_only(out_folder, rot_mat, root_trans, temporal_mask=None, bg_img=None, orig_width=2048, orig_height=1024, \
    orig_img=False, wireframe=False, sideview=False, dest_name_tag=None):
    # rot_mat: T X 24 X 3 X 3
    # root_trans: T X 3
    timesteps = rot_mat.shape[0]
    ...
    for frame_idx in range(timesteps):
        ...
        # print("mesh color:{0}".format(mesh_color))
        pred_v_list.append(pred_vertices[0].data.cpu().numpy())

        # print("init camera:{0}".format(init_cam.size()))
    mc = mesh_color
    img_list = renderer.render_for_interpolation(
        img,
        pred_v_list,
        temporal_mask,
        cam=init_cam[0].data.cpu().numpy(),
...
def debug_vis_rendered_mesh_only(out_folder, vertices, cam, orig_width=224, orig_height=224, orig_img=False, wireframe=False, sideview=False):
    # rot_mat: T X 24 X 3 X 3
    # root_trans: T X 3
    # timesteps = rot_mat.shape[0]
    timesteps = vertices.shape[0]
    renderer = Renderer(resolution=(orig_width, orig_height), orig_img=orig_img, wireframe=wireframe)
